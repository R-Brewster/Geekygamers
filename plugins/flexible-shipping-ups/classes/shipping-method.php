...
class Flexible_Shipping_UPS_Shipping_Method extends WC_Shipping_Method {
	...
	const NEGOTIATED_RATES_LABEL = 'negotiated_rates_label';
	...
	public function init_instance_form_fields() {
		$this->instance_form_fields = array(
			...
			self::NEGOTIATED_RATES_LABEL => array(
	...
	/**
	 * Mask password in request.
	 *
	 * @param array $rate_request_data .
	 *
	 * @return array
	 */
	protected function mask_password( array $rate_request_data ) {
		$security  = 'UPSSecurity';
		$user_name = 'UsernameToken';
		$password  = 'Password';
		if ( isset( $rate_request_data[ $security ] ) && isset( $rate_request_data[ $security ][ $user_name ] ) && isset( $rate_request_data[ $security ][ $user_name ][ $password ] ) ) {
			$rate_request_data[ $security ][ $user_name ][ $password ] = '****************';
		}
	...
	protected function get_rates( $package, $ups_access_point = null ) {
		...
		if ( current_user_can( 'manage_woocommerce' ) ) {
			$this->debug(
				...
				'<pre>' . print_r( $this->mask_password( $rate_request_data ), true ) . '</pre>' )
			);
		}
	...
	private function add_rates_from_ups_response(
		...
		if ( isset( $ups_rates_response->RateResponse ) && isset( $ups_rates_response->RateResponse->RatedShipment ) ) { // phpcs:ignore
			if ( ! is_array( $ups_rates_response->RateResponse->RatedShipment ) ) {
				$ups_rates_response->RateResponse->RatedShipment = [ $ups_rates_response->RateResponse->RatedShipment ];
			}
			if ( 'no' === $this->get_instance_option( 'custom_services', 'no' ) ) {
				...
				foreach ( $enabled_services as $enabled_service_code => $enabled_service ) {
					...
					foreach ( $ups_rates_response->RateResponse->RatedShipment as $ups_rate ) { // phpcs:ignore
						if ( (string) $enabled_service_code === (string) $ups_rate->Service->Code ) { // phpcs:ignore
							if ( $this->add_rate_from_ups_rate( $ups_rate, $rate_label_suffix, $rate_id_suffix, $meta_data ) ) {
								$added_rates ++;
							};
						}
